...
Evaluate::Evaluate() : Q_coll_(1000.f), Q_kine_(10000.f), orientation_infeasible_(0), T_norm_(100), A_norm_(PI), D_norm_(1.0), T_weight_(1), A_weight_(1), D_weight_(1) {}
...
void Evaluate::perform(ramp_msgs::EvaluationRequest& req, ramp_msgs::EvaluationResponse& res)
{
  ...
  /*
   * New - don't use probabilistic obstacles at all
   * Only use direct probability values 
   */
  // Sets orientation_infeasible_
  performFeasibility(req);
  //ROS_INFO("Done with performFeasibility");
...
void Evaluate::performFeasibility(ramp_msgs::EvaluationRequest& er) 
{
  ...
  cd_.performUsingCombinedMap(er.trajectory, er.obstacle_trjs, er.robot_radius, er.obstacle_cir_groups, combinedGrid_, hmap_, qr_);
...
void Evaluate::performFitness(ramp_msgs::RampTrajectory& trj, const double& offset, bool hmap, double& result) 
{
  ...
  if(trj.feasible)
  {
    ...
    // Get boundary cost and normalize it
    //ROS_INFO("Before getBoundaryCost");
    double bcost;
    /*getBoundaryCost(trj, bcost);
    bcost /= 4.94975;
    double bcost_cost = 1.0/bcost;
    ...
    //ROS_INFO("Weighted terms T: %f A: %f D: %f P: %f", Tterm, Aterm, Dterm, Pterm);

    // Compute overall cost
    cost = Tterm + Aterm + Dterm + Pterm + bcost;
  }
